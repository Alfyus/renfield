...
**Schicht-A-Fakten als Retrieval-Quelle:** Aus Dokumenten extrahierte strukturierte Fakten (`document_facts` — Steuernummer, Aussteller, Zahlungsverpflichtung) waren bis dato *write-only*: nichts las sie. `services/document_fact_retrieval.py` macht sie abfragbar und fusioniert `document_fact` als weitere Quelle in dieselbe RRF (grünes „Fakt"-Badge unter `/brain`). Keyword-FTS über `search_vector` plus ein Identifier-`ILIKE`-Zweig auf `normalized_value` (Postgres tokenisiert `114/5876/5293` unzuverlässig — der ILIKE-Zweig ist die verlässliche Exakt-Identifier-Suche, nur bei identifier-förmigen Query-Tokens aktiv). Fakten erben die Circle-Tier-Policy ihres Eltern-Dokuments. Zwei reine Backend-Lesepfade liegen daneben bereit (UI separat): `facts_for_document(doc_id)` (alle Fakten eines Dokuments) und `obligations(due_before)` — die Verpflichtungs-Agenda (Rechnungen + Behörden-Fristen, nächste Frist zuerst), das eigentlich tragende Versprechen gegen die „stiller-Archiv"-Narbe.
