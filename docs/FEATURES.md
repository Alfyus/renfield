...
- **Nachrichten-Branching / Edit-and-Fork** (`CHAT_BRANCHING_ENABLED`, opt-in/dark — Chat-UI Item 1, Phase 1; Design `docs/design/chat-branching.md`): Du kannst deine **letzte** Nachricht bearbeiten oder den **letzten** Assistenten-Turn neu generieren — statt die Historie zu überschreiben, entsteht ein **Branch** im Konversationsbaum. Datenmodell: `messages.parent_message_id` (Self-FK) + `conversations.active_leaf_message_id`; der aktive Zweig ist der rekursive Pfad von der Leaf-Nachricht aufwärts (eine rekursive CTE, konversations-gescopt). Vier Pfade werden branch-bewusst: Historie-Laden, `conv_context`-Replay (heilt von selbst), Erinnerungs-Extraktion (Memories eines verworfenen Zweigs werden `is_active=False` gesetzt → keine Konflation), und die Nachrichtensuche (auf den aktiven Zweig gefiltert + Index neu berechnet). Flag-aus = byte-identisch (CTE-always-on über den einmaligen Backfill). **Phase 2** (offen): Fork von *jeder* Nachricht + Branch-Umschalter (`‹2/3›`) + Branch löschen. Migration `pc20260618_message_branching`.
