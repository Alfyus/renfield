...
- **Frontend / GUI to surface facts — two of three surfaces remain.** WHAT: of the three layered surfaces, the `/brain` one shipped; the two dedicated surfaces are unbuilt. A user who opens a specific tax letter still sees no fact card, and there's no deadline inbox.
    1. **Obligations agenda** (the payoff — new page, e.g. `/brain/fristen`): deadline inbox grouped by urgency (Überfällig / diese Woche / später) off `obligation_date`, with `legal_gate` kinds flagged `⚑ rechtlich` and gated behind a mandatory **Bestätigen** step (golden-set rule: statutory kinds always human-confirmed). Reads the live `GET /api/atoms/obligations`. Pairs with the obligation notifier (item above).
    2. **Per-document facts panel** (cheapest first step — slots into the existing `/knowledge` document detail): a "Fakten" card listing issuer / total / identifiers / obligations, Paperless-metadata style. Reads the live `GET /api/atoms/documents/{id}/facts`.
  ...
  Cross-cutting (for surfaces 1 & 2): `TierBadge` on every fact (facts inherit document tier), a `source` provenance marker (deterministic `✓` = trustworthy vs LLM `~` = advisory), `confidence` could gate a "needs review" affordance, full `dark:` variants + de/en i18n. The retrieval dict already returns `source`, `confidence`, `legal_gate`, `obligation_date`, `amount_*`, `circle_tier` — the panels just render it. **Read `DESIGN.md` first** (crimson/turquoise/cream, Cormorant + DM Sans, `.tier-badge-{0..4}`, `.card`) — do not deviate without approval. **Process:** new agenda page → run `/add-frontend-page`, consider `/plan-design-review` before building. **Recommendation:** panel (2) first (lowest risk, existing page, proves the route end-to-end in the UI), then agenda (1) + the notifier together (the real prize). They stack; they don't compete.
