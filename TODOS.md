...
### Structured-memory reconciler — residual race hardening
Surfaced by `/review` of the `feature/structured-memory-kg` branch (2026-06-04). All three are low-probability, non-data-corrupting races; the merge invariant (tier=MIN), cross-user refusal, and ownership-404 all hold today. Build only if the reconciler runs at higher concurrency/volume than the single-boot per-user pass it ships with.
- **#3 — Concurrent approve of two overlapping proposals.** Two PENDING proposals can share an entity (A↔B and B↔C). If both are approved in parallel, the second `merge_entities` runs on a now-tombstoned side; `is_already_merged` guard catches it and returns None (no corruption), but the proposal still flips to APPROVED with a no-op merge. Owner sees "approved" with nothing merged. Fix: re-validate both sides are live canonical inside `approve_proposal` before flipping status, else mark `REJECTED` with a "superseded" note. **TRIGGER:** owners report "approved a merge but the duplicate is still there."
- **#4 — `run_reconciler` overlap.** No advisory lock around `run_for_user`; two overlapping runs for the same user re-find the same pairs. The `NOT EXISTS` pending-proposal filter + partial-unique on `kg_merge_proposals` dedupe proposals, and `touched` + `is_already_merged` dedupe auto-merges, so the outcome is correct but wasteful. Fix: `pg_advisory_xact_lock(hashtext('kg_reconciler:'||user_id))` at the top of `run_for_user`. **TRIGGER:** reconciler moves off the single-boot scheduler to a recurring/multi-worker cadence.
- **#6 — Embedding-null entities never reconciled.** `find_duplicate_pairs` requires `embedding IS NOT NULL` on both sides; entities created before embeddings backfilled (or when the embed call failed) are invisible to the self-join forever. Fix: a periodic embed-backfill for `kg_entities WHERE embedding IS NULL AND is_active`. **TRIGGER:** observed duplicate clusters that the reconciler never proposes, confirmed to have null embeddings.
