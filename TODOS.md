...
### Schicht A field extractor — follow-ups (out of `feat/schicht-a-field-extractor` PR scope)
The hybrid extractor (deterministic Steuernummer/IBAN with whitespace normalization + LLM obligations/universal facts) landed as a `post_document_ingest` consumer storing `document_fact` atoms. Opt-in: `schicht_a_extraction_enabled` (default False). These were scoped OUT of the first PR and confirmed during its `/review`.

- **Fact retrieval + obligation-alert consumer — the substantive next deliverable.** WHAT: facts are **write-only today** — nothing reads `document_fact` atoms. Build (a) a `DocumentFactRetrieval` module wired into `PolymorphicAtomStore.query()` so facts surface in `/brain` and agent context (RRF), and (b) the obligation-deadline notifier that turns `category=obligation` rows with a future `obligation_date` into reminders (legal_gate kinds → mandatory human-confirm per the golden set). WHY: the whole point of extracting deadlines is to act on them; the extractor is the producer half only. **Depends on:** the extractor PR merging. **Note:** when this lands, also wire `AtomService.update_tier` to call `resolver.invalidate_for_atom` for fact atoms (today it invalidates only the parent document atom — harmless while nothing reads facts through the resolver cache, a stale-read bug once retrieval exists).
- **Frontend / GUI to surface facts — facts are invisible to the user today.** WHAT: extraction runs and stores `document_fact` atoms, but there is **no UI** anywhere that shows them. A user who uploads a tax letter sees nothing change. Needs a consumer GUI. Three layered surfaces, driven by the two fact value-classes (obligations = actionable/time-bound; identifiers+universal = reference metadata):
    1. **Obligations agenda** (the payoff — new page, e.g. `/brain/fristen`): deadline inbox grouped by urgency (Überfällig / diese Woche / später) off `obligation_date`, with `legal_gate` kinds flagged `⚑ rechtlich` and gated behind a mandatory **Bestätigen** step (golden-set rule: statutory kinds always human-confirmed). This is the surface that pairs with the obligation-alert notifier.
    2. **Per-document facts panel** (cheapest first step — slots into the existing `/knowledge` document detail): a "Fakten" card listing issuer / total / identifiers / obligations, Paperless-metadata style. Proves the retrieval module end-to-end against an existing page.
    3. **Facts in `/brain` unified search** (near-free once retrieval exists): facts as a first-class atom type in `PolymorphicAtomStore.query()` RRF results, with a `Fakten` filter chip.
  Cross-cutting: `TierBadge` on every fact (facts inherit document tier), a `source` provenance marker (deterministic `✓` = trustworthy vs LLM `~` = advisory), `confidence` could gate a "needs review" affordance, full `dark:` variants + de/en i18n (both `de.json`/`en.json`). **Read `DESIGN.md` first** (crimson/turquoise/cream, Cormorant + DM Sans, `.tier-badge-{0..4}`, `.card`) — do not deviate without approval. **Depends on:** the fact-retrieval module above (all three surfaces read through it; nothing renders until facts are queryable). **Process:** non-trivial new surface → run `/add-frontend-page` for the agenda page, consider `/plan-design-review` before building. **Recommendation:** B first (lowest risk, existing page), then A (the real prize), then C (falls out for free). The three stack; they don't compete.
