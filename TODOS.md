...
- ~~**Fact retrieval (`DocumentFactRetrieval` + `/brain` RRF + `update_tier` cache invalidation).**~~ **SHIPPED #643 / v2.10.14.** Facts are queryable; the resolver-invalidation note is delivered (T5). The agent-context wiring (facts in the ReAct agent's retrieval context, not just `/brain`) is the one unbuilt slice — `internal.knowledge_search` / agent RAG still reads chunks only, not `document_fact`. **Trigger:** if the agent should cite extracted facts (e.g. "what's my Steuernummer") rather than the chunk they came from.
- **Obligation-deadline notifier — the load-bearing remaining half.** WHAT: turn `category=obligation` rows with a future `obligation_date` into proactive reminders (legal_gate kinds → mandatory human-confirm per the golden set). WHY: the whole point of extracting deadlines is to *act* on them — retrieval (shipped) makes them queryable; the notifier makes them arrive. The `obligations(due_before)` read path it builds on is already live (`GET /api/atoms/obligations`). Pairs with the obligations agenda UI (surface 1 below). **Trigger:** ready now — no external gate.
    - **Migrate the agenda's localStorage Bestätigt state onto this ledger.** The Schicht-A-GUI plan (eng-reviewed 2026-06-01) ships the agenda's "Bestätigen" acknowledgement in **localStorage, keyed by fact `id`** as a knowingly-accepted stopgap (D1/D4/D13). When this notifier builds its server-side `(obligation_id, milestone)` notified-ledger, migrate that client state onto it and reconcile/delete the localStorage store so there is one source of truth for "is this deadline handled" (two stores diverge across devices otherwise). Design per the cross-model learning `schicht-a-obligations-source-of-truth` (owner-targeted, legal kinds human-gated via `/brain/review`). **Depends on:** this notifier's server ledger.
- ~~**Frontend / GUI to surface facts — the three layered surfaces.**~~ **ALL THREE SHIPPED** (`/brain` in #643; the two dedicated surfaces built on `feat/schicht-a-gui`, eng + design + `/review` cleared 2026-06-01).
    ...
    3. ~~**Facts in `/brain` unified search.**~~ **SHIPPED #643** — `document_fact` is a fused RRF atom type with a green "Fakt" badge (`BrainPage`/`BrainReviewPage` + de/en i18n). A `Fakten` filter chip was NOT built (results are mixed-type today) — add if users want to scope to facts only.
  Delivered cross-cutting: shared `FactProvenance` (`✓` deterministic / `~` advisory + low-confidence hint) + `ObligationRow` + `TierBadge` on every fact; `.fact-group` / `.legal-flag` / `.toast` / `.atom-row--bestaetigt` tokens (DESIGN.md-compliant, `--color-cream`, dark + reduced-motion); full de/en i18n. **Still deferred:** the `Fakten` filter chip on `/brain`; per-fact inline `TierPicker` (tier follows the document via the existing `update_tier` cascade); `.ics` export.
...
- **Concurrent same-doc reindex can leave duplicate facts.** WHAT: the hook writes the new fact set then purges the old (write-then-purge, so a crash can't lose facts — but two overlapping reindexes of one `document_id` each write a fresh set → duplicates survive). Add a `SELECT … FOR UPDATE` / advisory lock on the document at hook start. WHY: dedup correctness under concurrency. **Cons:** reindex is already serialized per worker (one task at a time), so this is latent. **Trigger:** on observing duplicate `document_fact` rows for one document in practice.
- **`amount_currency` is truncated, not validated.** WHAT: an LLM-hallucinated currency string is capped to `String(8)` + uppercased, not checked against ISO-4217. Cosmetic data-quality. **Trigger:** if currency-typed queries land on the facts and bad values cause noise.
...
### Structured Memory Phase 4 — graph-expansion retrieval (RE-DEFERRED 2026-06-04)
Spec: `docs/HANDOVER_graph_expansion.md` §4. A candidate per-module MVP was built and **parked** on branch `feature/structured-memory-phase4-subsume` (commit `2794872`, `services/graph_expansion.py` + `tests/backend/test_graph_expansion_pg.py`, 6/6) but **`/plan-eng-review` + outside voice re-deferred it** — the per-module placement has interacting flaws. Do NOT resurrect the parked MVP as-is; rebuild on the **post-RRF single-insertion design** with these must-fixes:
- **Placement:** single `expand()` call in `polymorphic_atom_store.py` AFTER RRF (handover design), so decay survives and it runs once — NOT per-module in `get_relevant_context` + `get_relevant_atoms` (that double-works and lets RRF re-rank discard the decay).
- **Edge truncation (P0):** `get_relevant_atoms` rel-fetch is `LIMIT kg_max_context_triples` with **no ORDER BY** — after expansion inflates entities, connecting edges get truncated → orphan `kg_node` atoms + flaky tests. Raise the limit proportionally / ORDER BY so expanded nodes' edges survive.
- **Name-leak (P1, fix-now not defer):** relation tier = `LEAST(subj,obj)`, so a visible relation can name an **owner-only endpoint**; the `name_map` lookup (`kg_retrieval.py:~458`) is unfiltered. `kg_graph_service.focus` does it right (re-fetch endpoints through the circle gate, drop/"?" on miss). Expansion amplifies by fan-out → route name resolution through `kg_entities_circles_filter`.
- **Multi-pivot BFS (P1):** a per-hop frontier cap over a *multi-source* walk can drop the shortest path to a node and mislabel its hop distance → wrong decay. Track true min hop distance across pivots before capping.
- **Anonymous path:** `expand` must re-implement the `user_id is None → public-tier-only` branch independently; add a tier-leak test for the anonymous (not just authenticated-non-owner) case.
- **Provenance:** mark expanded (2-hop) nodes so the agent prompt doesn't present them with the same authority as directly-matched facts (hallucination-surface guard).
