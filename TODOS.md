...
**NOT BUILT (remaining):**
- 🟡 **(1) Message branching / edit-and-fork** (T1) — **Phase 1 SHIPPED** (`feat/chat-branching-phase1`, `CHAT_BRANCHING_ENABLED`, dark; design `docs/design/chat-branching.md`). Conversation-tree schema (`messages.parent_message_id` self-FK + `conversations.active_leaf_message_id`, migration `pc20260618_message_branching` + idempotent backfill), recursive active-path CTE (conversation-scoped recursive step), all four branch-aware seams (history / conv_context self-heal / memory-deactivate-at-fork / search filter+reindex), `save_message` tree maintenance, WS `fork_from_message_id` (edit/regenerate by target role), `PUT …/active-leaf`, frontend edit-latest-user / regenerate-latest-assistant. CTE-always-on; flag-off byte-identical. **/review caught + fixed 2 P1s** (cross-conversation IDOR; conversation-delete FK violation), regression-tested; 16 backend tests pass on real PG. **Phase 2 (remaining):** fork-from-ANY message + the `‹2/3›` branch switcher (`ChatHeader.tsx`) + delete-branch; the switcher needs memory **reactivation** (deactivate-at-fork is only Phase-1-safe because you can't navigate back). Documented deactivate-at-fork background-extraction race.
