...
### Hinzugefuegt
- **Fix: Bridge-Resolver-Konflation** — `resolve_entity` bekommt `use_embedding` (Default True). Der Memory→KG-Bridge-Pfad (save-time + Backfill) nutzt jetzt `use_embedding=False`: bloße Vornamen ("Jutta") embedden nah an andere gleich-tier Personen ("Anna Johanna von den Bongard") und wurden sonst in die FALSCHE Person gefaltet (die ursprüngliche Konflation, auf Resolver-Ebene). Bridge resolved nun exact/surface → sonst NEU anlegen; echte Near-Dupes bleiben dem review-gated Reconciler. Verifiziert: use_embedding=False legt eigene Jutta-Entität an.
- **Memory-Extraktion: Subjekt-Schaerfung** — der v1-Extraktions-Prompt (`prompts/memory.yaml`, DE+EN) erzwingt jetzt korrekte `subject`-Belegung: echte Eigennamen als Subjekt, NIEMALS Pronomen ("sie") oder Rollen-/Beziehungsfloskeln ("meine Frau", "der Benutzer") (die sonst via Bridge Junk-KG-Entitaeten erzeugt haetten) → stattdessen `null`; Few-Shot-Beispiele inkl. benannter Dritter. Gemessen am Live-Modell (qwen3.6): Pronoun/Rollen-Leaks 2→0, named-subject 4/4. Neue Eval `bin/run_memory_subject_eval.py` + `tests/eval/memory_subject_extraction_eval.yaml` (in-Pod, gegen das Prod-Modell). Voraussetzung dafuer, dass Bridge/Subsume real greifen.

- **Structured Memory — KG als Memory-Substrat.** Persoenliches Memory wird von flachem Text auf ein typisiertes Entitaets-/Relations-Substrat mit Provenienz gehoben. (1) **Subjekt-Attribution** (`subject_name`/`subject_entity_id` auf `conversation_memories`): jeder Fakt traegt, ueber WEN er handelt; das Retrieval taggt den injizierten Kontext (`- [FACT · <Name>] …`), sodass das LLM Fakten ueber verschiedene Personen nicht mehr vermischt (behebt die gemeldete Konflation). (2) **Kanonisierung**: `kg_entities` bekommt `canonical_id` (Merge-Tombstone-Pointer), `surface_forms` (Aliase, GIN), `entity_types` (Multi-Typ; skalarer `entity_type` bleibt Primaer), `external_id`. `resolve_entity` als Kaskade exakt → Surface-Form → Embedding (same-tier, halfvec) → neu; `merge_entities` absorbiert + reparentiert Relationen + Tier=MIN (eine Verschmelzung erhoeht nie die Sichtbarkeit). (3) **Reconciler** (`KG_RECONCILER_ENABLED`, opt-in): periodischer Per-User-Self-Join — same-tier high-confidence Dubletten werden auto-gemerged, Cross-Tier/Grauzone landen als `kg_merge_proposals` in der Owner-Review (`/brain/review`, neue Merge-Vorschlag-Karte mit Survivor-Toggle, Cross-Tier-Warnung, 5s-Undo). Review-Routen laufen auf `KG_VIEW` (eigener Graph + Per-Proposal-Ownership-404). Jeder Lauf ist per-User durch einen Advisory-Lock serialisiert, bettet zuerst bis zu `KG_RECONCILER_EMBED_BACKFILL_PER_RUN` Null-Embedding-Entitaeten nach und schliesst ein durch parallele Genehmigung ueberholtes Proposal als `superseded`. (4) **Provenienz**: Relationen tragen `stated_by_user_id` (wer hat es gesagt). Neue Settings `KG_RECONCILER_*`. Migrationen `pc20260604_struct_mem` + `pc20260604b_kgmp`. KG-Extraktions-Eval unter `bin/run_kg_extraction_eval.py`.
- **Structured Memory Phase 3 — Memory→KG-Bridge** (`MEMORY_KG_BRIDGE_ENABLED`, opt-in, dark by default). Macht „Was weiß ich über X" deterministisch, indem flache Memories an kanonische Entitaeten gebunden werden. (3a) `resolve_entity` wird bridge-sicher: additive `create_tier` (ersetzt das hartkodierte Tier-0 fuer Create + Same-Tier-Embedding-Suche) + `match_entity_type` (typ-gescopte Exact-/Surface-/Embedding-Aufloesung), beide rueckwaertskompatibel; Reconciler-**Same-Name-Gate** (gleicher Name + leere/identische Description ⇒ Review statt Auto-Merge, schliesst die Konflations-Rueckkopplung). (3b) Save-time-Bridge im Hintergrund-Extraktionspfad + Backfill (`bin/backfill_subject_entity_ids.py`, `--dry-run`/`--commit`, resolve-or-create @memory.tier, per-Row-atomar, idempotent). (3c) Entity-augmentiertes Retrieval: im Query genannte Entitaeten (Exact+Surface-Form, kein LLM) unionen ihre subjekt-verlinkten Memories deterministisch in die Embedding-Treffer (similarity-Floor, `MEMORY_RETRIEVAL_SUBJECT_UNION_LIMIT`, canonical_id-Tombstone-Chase) — **durch denselben `circle_sql`-Filter**; `GET /api/memory/by-subject/{entity_id}` + Subjekt/Entity-Anzeige im `/memory`-Card und `/wissen`-Drawer. Keine neue Migration. Aus = byte-identisch. (3-subsume, `MEMORY_SUBSUME_TO_KG`, opt-in/aggressiv) zerlegbare `fact`-Memories mit Subjekt werden nicht mehr flach dupliziert (leben nur im KG); Praeferenzen/Instruktionen/Kontext bleiben flach. Phase 4 (Graph-Expansion) als Post-RRF-Rebuild umgesetzt — s. naechster Eintrag.
- **Structured Memory Phase 4 — Graph-Expansion-Retrieval** (`GRAPH_EXPANSION_ENABLED`, opt-in, dark by default). `services/graph_expansion.py::expand_fused` laeuft **nach** der RRF-Fusion in `PolymorphicAtomStore.query`: nimmt die fused `AtomMatch`-Liste, expandiert die obersten `kg_node`-Pivots 1-`GRAPH_EXPANSION_MAX_HOPS` Hops ueber `kg_relations` und haengt Decay-bewertete Nachbar-`kg_node`- + leak-sichere `kg_edge`-Atome an. Level-synchrone BFS (korrekte Min-Hop-Distanz), Circle-Filter pro Hop, Frontier-Cap (Hub-Schutz), Kanten nur bei beidseitig sichtbaren Endpunkten, Provenienz (`payload.expanded`+`hop`). Einzelner Insertion-Point (kein Doppel-Work, Decay ueberlebt). Per-Module-MVP wurde nach `/plan-eng-review` + Outside-Voice verworfen (P0/P1-Flaws), Rebuild auf dem Handover-Design (`docs/HANDOVER_graph_expansion.md` §4). Keine Migration. Aus = `query` byte-identisch.
...
Drei latente Bugs, die das Self-Learning-System in Produktion stillschweigend lahmlegten — gefunden bei der End-zu-End-Validierung und jeweils gefixt, deployt und live verifiziert:

- **Skill-Injection lief auf jedem Agent-Turn ins Leere** (`v2.12.10`) — `SkillService.find_similar` band `:asker` in einem nackten `:asker IS NULL`-Zweig; asyncpg brach den Parse mit „could not determine data type of parameter $3" ab. Der Fehler wurde als Warnung geschluckt, sodass nie ein Skill (auch kein Seed) in den Agent-Prompt injiziert wurde — trotz `SKILLS_ENABLED=true`. `:asker` wird nun an jeder Stelle nach `INTEGER` gecastet; Postgres-only Regressionstest ergänzt (der sqlite-Harness führt keinen server-seitigen Parse aus, daher blieb der Bug dort unsichtbar). PR [#676](https://github.com/ebongard/renfield/pull/676).
- **Tägliche Hintergrund-Scheduler liefen nie auf kurzlebigen Pods** (`v2.12.11`) — `_spawn_periodic_task` schlief erst ein Intervall, bevor `work()` lief; bei `interval=86400s` feuerte der erste Tick also erst 24 h nach Boot und der Timer startete bei jedem Pod-Neustart bei null. Skill Curator, Trajectory Cleanup, Shadow-Log-Cleanup und Speaker-Vocab-Rebuild liefen damit faktisch nie. Opt-in `run_at_boot` führt einen Tick direkt nach dem Spawn aus. PR [#680](https://github.com/ebongard/renfield/pull/680).
- **`agent_trajectories` blieb dauerhaft leer und Skill-Auto-Extraktion feuerte nie** (`v2.12.12`) — der Agent-Loop hängte den terminalen `final_answer`-Step nicht an `context.steps` an (nur `tool_call`/`tool_result`/`error`). Da die Post-Turn-Verbuchung den Turn aus `context.steps` klassifiziert, wurde jeder Turn als `ABORT` gewertet: die Trajektorie wurde verworfen (Default-Capture-Set `success,tool_fail` schließt `abort` aus) und `turn_success` war immer `False` — also keine Skill-Auto-Extraktion, und jeder injizierte Skill wurde fälschlich als Fehlschlag verbucht (Auto-Demote-Risiko). Der terminale Step wird jetzt im `run()`-Wrapper aufgezeichnet. PR [#682](https://github.com/ebongard/renfield/pull/682).
