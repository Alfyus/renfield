...
describe('ChatMessages — chat branching (Phase 1)', () => {
  ...
  it('only the LATEST user message is editable (Phase 1 gate)', async () => {
    enableBranchingFlag();
    // Two user turns; only the second (index 2) should get an Edit button.
    ...
    // Exactly one Edit affordance (on the latest user message).
    const editBtns = await screen.findAllByLabelText(/Nachricht bearbeiten|Edit message/i);
    expect(editBtns).toHaveLength(1);
    // Exactly one Regenerate affordance (on the latest assistant turn).
